    ...
    >
      <div className="max-w-[1280px] mx-auto flex items-center justify-between gap-4 ">
          ...
          {navigationItems.map((item, idx) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "py-3 text-sm whitespace-nowrap rounded-lg transition-all backdrop-blur-[200px] text-center flex-1 min-w-[145px]",
                  isActive
                    ? "bg-[#FFFFFFE5] text-[#EF9000] font-semibold"
                    : "bg-[#FFFFFF4D] text-[#737373] font-normal",
                )}
              >
                {item.label}
              </Link>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <SheetContent
            side="right"
            className="w-[300px] bg-white border-none p-6 pt-16 [&>button]:hidden overflow-y-auto"
          >
